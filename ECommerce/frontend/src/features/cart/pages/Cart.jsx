  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <SectionHeading
        ...
        title="Your cart"
        action={
          <button onClick={() => navigate('shop')} className="text-sm font-medium text-[#62574D] transition hover:text-[#11110F]" type="button">
            Continue shopping
          </button>
        }
      />
            ...
            {cartItems.map((item) => (
              <div key={item.id} className="flex flex-col gap-4 rounded-3xl border border-black/5 bg-[#F7F3EC] p-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex items-center gap-3 sm:flex-col sm:items-end">
                  ...
                  <button onClick={() => removeFromCart(item.id)} className="text-sm font-medium text-[#B04A3A]" type="button">
                    Remove
                  </button>
                </div>
              </div>
